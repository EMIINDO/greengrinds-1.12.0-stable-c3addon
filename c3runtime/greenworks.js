...
if (process.platform == 'darwin') {
  greenworks = require(__dirname + '/greenworks-osx');
} else if (process.platform == 'win32') {
  if (process.arch == 'x64')
    greenworks = require(__dirname + '/greenworks-win64');
  else if (process.arch == 'ia32')
    error("Greengrinds is not supported on 32-bit Windows.");
} else if (process.platform == 'linux') {
  if (process.arch == 'x64')
    greenworks = require(__dirname + '/greenworks-linux64');
  else if (process.arch == 'ia32')
    error("Greengrinds is not supported on 32-bit Linux.");
  else
    error("Greengrinds is not supported on " + process.platform + " " + process.arch + ".");
}
