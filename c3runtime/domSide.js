  ...
  const HANDLER_CLASS = class GreenworksDOMHandler extends DOMHandler {
    constructor(iRuntime) {
      ...
      this.AddRuntimeMessageHandlers([
        ...
        ["request-encrypted-app-ticket", (e) => this._OnRequestEncryptedAppTicket(e)]
      ]);
    }
    _Load(e) {
      ...
      if (this._isNWjs || this._isElectron) {
        try {
          ...
          if (this._greenworks["on"]) {
            ...
            this._greenworks["on"]("micro-txn-authorization-response", (appID, orderID, authorized) => {
              this.PostToRuntime("micro-txn-authorization-response", {
    ...
    requestEncryptedAppTicket(e) {
      return new Promise((resolve, reject) => {
        this._greenworks["getEncryptedAppTicket"](
          e["userData"],
          (encryptedTicket) => {
            console.log('*** INFO *** domSide.js requestEncryptedAppTicket encryptedTicket', encryptedTicket);
            // Convert Buffer directly to base64 string
            const ticketString = encryptedTicket.toString('base64');
            resolve({ ticket: ticketString });
          },
          (error) => {
            console.error('*** ERROR *** Failed to get encrypted app ticket:', error);
            reject(error);
          }
        );
      });
    }
    async _OnRequestEncryptedAppTicket(e) {
      try {
        const result = await this.requestEncryptedAppTicket(e);
        this.PostToRuntime("request-encrypted-app-ticket-response", { 
          ticketString: result.ticket 
        });
      } catch (error) {
        console.error("Failed to request encrypted app ticket:", error);
        this.PostToRuntime("request-encrypted-app-ticket-response", { 
          error: error.message,
          ticketString: "" 
        });
